...
export default function Header() {
  ...
  // State to manage categories and dropdown status
  const [categories, setCategories] = useState<Category[]>([]); // Array of categories
  const [activeDropdown, setActiveDropdown] = useState<"category" | "user" | "notification" | "create" | null>(null);
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const data = await fetcher<Category[]>("/categories"); // Define the expected response type
        setCategories(data);
      } catch (err) {
  ...
  const toggleDropdown = (type: "category" | "user" | "notification" | "create") => {
