...
const AdminCategoriesPage: React.FC = () => {
  ...
  const [filters, setFilters] = useState<{ idFrom: string; idTo: string }>({
    idFrom: "",
    idTo: "",
  });
  ...
  const [categoriesPerPage, setCategoriesPerPage] = useState<number | "all">(3);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allCategories = await fetcher<Category[]>("/categories", {
          headers,
        });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await fetcher<void>(`/categories/${categoryToDelete}`, {
        method: "DELETE",
        headers,
      });
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="table-auto w-full text-left">
          <thead className="bg-gray-100">
            <tr>
              {(["id", "title", "description"] as (keyof Category)[]).map(
                (key) => (
                  <th
                    key={key}
