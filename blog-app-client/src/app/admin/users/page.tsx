...
// Define types for user and filters
interface User {
  id: number;
  name: string;
  email: string;
...
const AdminUsersPage: React.FC = () => {
  ...
  const [searchBy, setSearchBy] = useState<"name" | "email" | "about">("name");
  ...
  const [usersPerPage, setUsersPerPage] = useState<number | "all">(3);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const allUsers = await fetcher<User[]>("/users", { headers });
  ...
  const applySearchFilter = (data: User[]): User[] => {
    ...
    return data.filter((user) => {
      switch (searchBy) {
        ...
        case "about":
          return user.about?.toLowerCase().includes(lowerCaseSearch) || false;
        default:
          return true;
      }
    });
  ...
  const sortedUsers = [...filteredUsers].sort((a, b) => {
    ...
    // Perform comparison for defined values
      ...
      <div className="flex justify-between items-center bg-white p-4 shadow-md rounded-lg mb-4">
        <div className="flex items-center gap-4">
          <select
            value={searchBy}
            onChange={(e) =>
              setSearchBy(e.target.value as "name" | "email" | "about")
            }
