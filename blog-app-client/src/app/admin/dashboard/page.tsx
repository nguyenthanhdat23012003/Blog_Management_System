  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const posts = await fetcher<Post[]>("/blogs", { headers });
        const categories = await fetcher<{ length: number }>("/categories", {
          headers,
        });
        const users = await fetcher<{ length: number }>("/users", { headers });
        const series = await fetcher<Series[]>("/series", { headers });
      ...
      <aside className="w-1/4 bg-gradient-to-br from-blue-400 to-indigo-600 text-white p-6 rounded-3xl shadow-md">
        ...
        <nav className="space-y-6">
          <a
            ...
            className="block font-medium hover:text-indigo-200"
          >
            Dashboard
          </a>
          <a
